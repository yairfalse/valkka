defmodule Valkka.Workspace do
  ...
  def scan do
    roots = Application.get_env(:valkka, :workspace_roots, [])
    depth = Application.get_env(:valkka, :scan_depth, 1)
    ...
    Enum.each(repos, fn repo ->
      case Valkka.Repo.Supervisor.open(repo.path) do
        {:ok, _pid} ->
  ...
  def list_repos do
    Registry.select(Valkka.Repo.Registry, [{{:"$1", :"$2", :_}, [], [{{:"$1", :"$2"}}]}])
    |> Enum.map(fn {path, pid} ->
      case Valkka.Repo.Worker.get_state(pid) do
        {:ok, state} -> state
  ...
  defp scan_root(root, depth) do
    case Valkka.Git.Native.workspace_scan(root, depth) do
